...
The resources saw some changes.

Access to the resources was simplified by provinding the equivalent for FindResource and FindResourceEx APIs. FindResource will find the first data entry matching the type and name (ignoring the language).

- Added new APIs mirroring `FindResource` and `FindResourceEx`.
- Added convenience helper methods to manually walk the directory tree.

You can now read out the full icons and cursors from the resources!

- Added Group Icon and Cursor parser.
- Added an example to show how to extract icons from a PE image.

Some fixes to the VersionInfo parser.

- Added an example to show how to use the VersionInfo parser.
- Fixed bug that could cause the VersionInfo parser in an infinite loop.
- Changed version_info getter to no longer hardcode 1033 language and instead take the first one it finds.
...
```rust
static IMAGE: Align512<[u8; X]> = Align512(include_bytes!("binary.dll"));
```
